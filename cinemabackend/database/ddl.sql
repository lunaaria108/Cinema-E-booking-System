...
CREATE TABLE email_verification_token (
    ...
    expires_at TIMESTAMP NOT NULL,
    created TIMESTAMP NOT NULL DEFAULT NOW(),
    CONSTRAINT fk_verified_user FOREIGN KEY (user_id) REFERENCES Users(user_id) ON DELETE CASCADE
);
...
CREATE TABLE Movie (
    ...
    status VARCHAR(20) NOT NULL CHECK (
        status IN ('Currently Running', 'Coming Soon', 'Ended')
    ),
    CONSTRAINT fk_movie_genre FOREIGN KEY (genre_id) REFERENCES Genre(genre_id) ON DELETE RESTRICT
);
CREATE TABLE Review (
...
CREATE TABLE Showtime (
    ...
    CONSTRAINT fk_showtime_hall FOREIGN KEY (hall_id) REFERENCES Hall(hall_id) ON DELETE RESTRICT
);
...
CREATE TABLE Booking (
    ...
    CONSTRAINT fk_booking_showtime FOREIGN KEY (showtime_id) REFERENCES Showtime(showtime_id) ON DELETE RESTRICT,
    CONSTRAINT fk_booking_promotion FOREIGN KEY (promotion_id) REFERENCES Promotion(promotion_id) ON DELETE
    SET NULL
);
CREATE TABLE Ticket (
...
CREATE TABLE Payment (
    ...
    payment_status VARCHAR(20) NOT NULL DEFAULT 'Pending' CHECK (
        payment_status IN ('Pending', 'Approved', 'Declined', 'Refunded')
    ),
    payment_reference VARCHAR(255),
    CONSTRAINT fk_payment_booking FOREIGN KEY (booking_id) REFERENCES Booking(booking_id) ON DELETE CASCADE,
    CONSTRAINT fk_payment_card FOREIGN KEY (card_id) REFERENCES PaymentCard(card_id) ON DELETE
    SET NULL
);
CREATE TABLE FavoriteMovie (
    ...
    CONSTRAINT fk_favorite_movie FOREIGN KEY (movie_id) REFERENCES Movie(movie_id) ON DELETE CASCADE,
...
CREATE TABLE UserPreference (
    ...
    CONSTRAINT fk_pref_genre FOREIGN KEY (preferred_genre_id) REFERENCES Genre(genre_id) ON DELETE
    SET NULL
);